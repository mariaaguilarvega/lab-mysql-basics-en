...
-- Query 13 -- In the client table, of all districts with a district_id lower than 10, how many clients 
...
SELECT account_id, SUM(amount) as total_loans_amount
FROM loan
GROUP BY account_id
ORDER BY total_loans_amount DESC
LIMIT 10;
...
-- Query 18 -- In the trans table, for account_id 396, sum the amount of transactions for each type (VYDAJ = Outgoing, PRIJEM = Incoming). Your output should have the account_id, 
